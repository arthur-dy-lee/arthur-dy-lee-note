...
JDBC是桥接模式的典型实现：Driver，MysqlDriver，OracleDriver。
...
ThreadLocalMap是当前线程的属性，map的key是当前ThreadLocal对象。 所以，get其实是一个根据key查找value的方法，只是存储的map，隐藏在当前Thread里面。

线程池共享变量延伸：InheritableThreadLocal，TransmittableThreadLocal

https://blog.csdn.net/agonie201218/article/details/125933740
...
2、客户端cookie存标记位
![](pics/read-write-consistency.jpeg)
...
https://blog.csdn.net/zhoutao198712/article/details/7791969
...
2. 非阻塞I/O：
6. 再加上Redis自身的事件处理模型将epoll中的连接，读写，关闭都转换为了时间，不在I/O上浪费过多的时间。
7. 单线程避免了线程切换和竞态产生的消耗。
...
Container runtime负责镜像管理以及Pod和容器的真正运行（CRI）；.0
...
(3)epoll==>时间复杂度O(1)
epoll可以理解为event poll，不同于忙轮询和无差别轮询，epoll会把哪个流发生了怎样的I/O事件通知我们。所以我们说epoll实际上是事件驱动（每个事件关联上fd,fd存放文件描述符）的，此时我们对这些流的操作都是有意义的。（复杂度降低到了O(1)）
...
epoll作为I/O多路复用技术
实现：红黑树和链表。维护了一下eventpoll，'链表'中只有活跃的连接，事件放到'红黑树'中（效率lgN），不想处理中，删除事件即可。当读取一个事情时，链表中自然就没有了，当操作系统接收到一个报文的时候，链表就会增加一个新的元素。添加/修改/删除事件放到红黑树中，效率是lgN, 获取句柄的时候，只是去链表中读取出来，从用户态到内核态。
使用：创建；操作：添加/修改/删除；获取句柄；关闭

[epoll的优劣和原理](https://time.geekbang.org/course/detail/100020301-68394)
...
select/poll/epoll三者产生，随着处理的文件描述符越多性能差异越大的原因
依据有三个
