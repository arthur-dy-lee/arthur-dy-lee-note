...
CPU缓存，其中L1是L2的子集，L2是L3的子集，L1到L3缓存容量依次增大，查找耗时依次增大，CPU查找顺序依次是L1、L2、L3、主存。
...
Java 的数组的确是放在一块连续内存里的，否则不可能做到在 O(1) 时间复杂度内存取元素，每个元素占用相同大小的内存空间。当你知道要访问的元素的索引时，可以直接计算出该元素在内存中的位置。
